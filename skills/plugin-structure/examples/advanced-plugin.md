...
```json
{
  "mcpServers": {
    "kubernetes": {
      "command": "node",
      "args": ["PLUGIN_ROOT/servers/kubernetes-mcp/index.js"],
      "env": {
    ...
    "terraform": {
      "command": "python",
      "args": ["PLUGIN_ROOT/servers/terraform-mcp/main.py"],
      "env": {
    ...
    "github-actions": {
      "command": "node",
      "args": ["PLUGIN_ROOT/servers/github-actions-mcp/server.js"],
      "env": {
...
\`\`\`javascript
const { DatadogClient } = require('PLUGIN_ROOT/lib/integrations/datadog')
...
\`\`\`javascript
const { SlackClient } = require('PLUGIN_ROOT/lib/integrations/slack')
...
\`\`\`bash
bash PLUGIN_ROOT/skills/kubernetes-ops/scripts/validate-manifest.sh deployment.yaml
\`\`\`
```
