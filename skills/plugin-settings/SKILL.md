...
description: This skill should be used when the user asks about "plugin settings", "store plugin configuration", "user-configurable plugin", ".local.md files", "plugin state files", "read YAML frontmatter", "per-project plugin settings", or wants to make plugin behavior configurable. Documents the .agent/plugin-name.local.md pattern for storing plugin-specific configuration with YAML frontmatter and markdown content.
version: 0.1.0
---

# Plugin Settings Pattern for coding assistant Plugins

## Overview

Plugins can store user-configurable settings and state in `.agent/plugin-name.local.md` files within the project directory. This pattern uses YAML frontmatter for structured configuration and markdown content for prompts or additional context.

**Key characteristics:**
- File location: `.agent/plugin-name.local.md` in project root
...
STATE_FILE=".agent/my-plugin.local.md"
...
Steps:
1. Check if settings exist at `.agent/my-plugin.local.md`
...
Check for plugin settings at `.agent/my-plugin.local.md`.
...
```bash
#!/bin/bash
STATE_FILE=".agent/security-scan.local.md"
...
**Use case:** Enable/disable hooks without editing hook-config.json (requires restart).
...
2. Create `.agent/my-plugin.local.md` with YAML frontmatter
3. Set appropriate values based on user input
4. Inform user that settings are saved
5. Remind user to restart coding assistant for hooks to recognize changes
```
...
Create `.agent/my-plugin.local.md` in your project:
...
After creating or editing, restart coding assistant for changes to take effect.
```
...
✅ **DO:**
- Use `.agent/plugin-name.local.md` format
- Match plugin name exactly
- Use `.local.md` suffix for user-local files

❌ **DON'T:**
- Use different directory (not `.agent/`)
...
**Important:** Settings changes require coding assistant restart.
...
3. Add gitignore entry for `.agent/*.local.md`
...
7. Remind users that changes require coding assistant restart
