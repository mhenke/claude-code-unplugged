...
```markdown
.agent/plugin-name-workflow.local.md:
...
Creating: .agent/feature-complete.flag
...
if [ -f .agent/feature-complete.flag ]; then
...
if [ -f .agent/deployment.lock ]; then
...
Written to .agent/deployment-state.local.md
...
rm .agent/deployment-state.local.md
