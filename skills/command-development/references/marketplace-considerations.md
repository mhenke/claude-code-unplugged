...
if [ ! -f ".agent/command-initialized" ]; then
...
Checking for user config: .agent/plugin-name.local.md

if [ -f ".agent/plugin-name.local.md" ]; then
  # Parse YAML frontmatter for settings
  VERBOSE=$(grep "^verbose:" .agent/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  COLOR=$(grep "^color:" .agent/plugin-name.local.md | cut -d: -f2 | tr -d ' ')
  MAX_RESULTS=$(grep "^max_results:" .agent/plugin-name.local.md | cut -d: -f2 | tr -d ' ')

  echo "✓ Using user configuration"
else
  echo "Using default configuration"
  echo "Create .agent/plugin-name.local.md to customize"
fi
...
Or set in .agent/plugin-name.local.md:
\`\`\`yaml
---
format: yaml
output: custom.txt
...
if [ -f ".agent/operation-completed.flag" ]; then
  ℹ️  Operation already completed

  Completed at: $(cat .agent/operation-completed.flag)

  To re-run:
  1. Remove flag: rm .agent/operation-completed.flag
  2. Run command again
...
Marking complete...
echo "$(date)" > .agent/operation-completed.flag
```
